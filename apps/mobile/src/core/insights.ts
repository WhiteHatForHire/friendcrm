...
export function buildBrief(data: CrmData, person: Person) {
  ...
  const hasContext = Boolean(person.summary || memories.length || loops.length || notes.length || moves.length);

  return {
    snapshot: person.summary ?? `${person.name} has no official story yet. Add a note or quick memory before trusting this file.`,
    ...
    nextMove: moves[0]?.draft ?? (hasContext ? "Choose one small, direct reason to reach out." : "No recommendation yet. Add a note, memory, loose thread, or next move first.")
  };
}
